...
function createDataSourceTasks({
  ...
  Object.keys(dataSourceTemplates).map(index => {
    ...
    if (template.type === DataSource.DynamoDB) {
      if (dataSources.find(_template => _template.name === template.name)) {
      } else {
              ...
              switch (dataSource.type) {
                case DataSource.DynamoDB:
                  if (foundDynamoDbDataSource === true) {
                    ...
                    return {
                      title: `DynamoDB: ${dataSourceConfig.tableName}`,
                      task: () =>
                ...
                case DataSource.None:
                  throw new Error(
                    `Unsupported data source: ${dataSource.name} Type: ${
                      dataSource.type
                    }`,
                  );
                default:
                  throw new Error(
                    `Unknown data source: ${dataSource.name} Type: ${
                      dataSource.type
                    }`,
                  );
              }
